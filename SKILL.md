...
- `doctor` and `doctor --json` are non-live, do not open a token or native store, and do not prove backend availability. `doctor --installation-check` may run the CLI version command and one isolated native-store probe but never reads an account or PAT. Only `doctor --account <name> --live` authorizes reading and authenticated validation of the selected account.
