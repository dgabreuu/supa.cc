...
identify `0.5.7` without credentials, local paths, caches, or build artifacts.

## 2. Confirm the 0.5.7 publication contract
...
## 3. Publish the stable GitHub Release for 0.5.7

After the candidate is merged and CI passes, create `v0.5.7` on the exact
candidate commit and publish a stable, non-draft, non-prerelease GitHub Release
using the `0.5.7` changelog entry. Do not attach local build artifacts.

## 4. Publish 0.5.7 to PyPI with OIDC
...
## 5. Verify pipx for 0.5.7 on Linux and Windows

Require the release workflow's Linux and Windows jobs to install
`supa.cc==0.5.7` from PyPI and pass both `supa.cc --version` and
`supa.cc version` after bounded index-propagation retries.

## 6. Promote the Homebrew formula after publication

Only after `v0.5.7` and PyPI are public, download the immutable GitHub source
...
Homebrew availability. Keep the changelog comparison at `v0.5.6...v0.5.7`.
