    ...
    for name in names:
        if not is_valid_account_name(name):
            raise AccountIndexInvalidError(
                "The local account index contains an invalid name."
            )
        if name not in seen:
            seen.add(name)
...
def safe_load_json_index(path: Path) -> Optional[List[str]]:
    """Read the index; None means only that it does not exist."""
    try:
...
class AccountStore:
    ...
    def _backup_failure(self) -> AccountTransactionError:
        return AccountTransactionError(
            "The transaction's temporary credential could not be verified safely."
        )

    def create_account_backup(self, name: str) -> None:
        """Copy the credential to secure storage outside the public index."""
    ...
    def save_account(self, account: Account) -> None:
        """Save and verify a credential without changing index names."""
        self._ensure_initialized()
        self._write_token_verified(account)

    def get_account(self, name: str) -> Optional[Account]:
        """Retrieve a credential directly from authoritative storage."""
        token = self._read_token_uncached(name)
        if token:
    ...
    def update_index(self, names: List[str]) -> None:
        """Replace the index under a lock without overwriting invalid content."""
        unique_names = _validated_unique_names(names)
    ...
    def add_account(self, account: Account) -> None:
        """Write the credential and name as one serialized logical transaction."""
        with self._index_lock():
    ...
    def remove_account(self, name: str) -> None:
        """Remove the credential and name with best-effort rollback under a lock."""
        with self._index_lock():
            names = self._read_index_locked()
