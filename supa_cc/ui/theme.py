import questionary

# Paleta de cores: escala de cinza + rosa choque
COLOR_BLACK = "#000000"
COLOR_DARK_GRAY = "#1A1A1A"
COLOR_MEDIUM_GRAY = "#4A4A4A"
COLOR_GRAY = "#808080"
COLOR_LIGHT_GRAY = "#B0B0B0"
COLOR_WHITE = "#FFFFFF"
COLOR_SHOCKING_PINK = "#FF1493"

# Estilos Rich centralizados
RICH_STYLES = {
    "title": f"bold {COLOR_WHITE}",
    "subtitle": f"bold {COLOR_LIGHT_GRAY}",
    "success": f"bold {COLOR_WHITE}",
    "error": f"bold {COLOR_SHOCKING_PINK}",
    "warning": f"bold {COLOR_LIGHT_GRAY}",
    "info": f"bold {COLOR_GRAY}",
    "border": COLOR_MEDIUM_GRAY,
    "highlight": f"bold {COLOR_SHOCKING_PINK}",
    "active": COLOR_WHITE,
    "inactive": COLOR_GRAY,
    "dim": COLOR_GRAY,
    "banner": f"bold {COLOR_SHOCKING_PINK}",
}

# Banner Supa.cc
SUPA_CC_BANNER = r"""
 ____
/ ___| _   _ _ __   __ _   ___ ___
\___ \| | | | '_ \ / _` | / __/ __|
 ___) | |_| | |_) | (_| || (_| (__
|____/ \__,_| .__/ \__,_(_)___\___|
            |_|
""".strip("\n")

# Estilo Questionary com paleta cinza + rosa choque
QUESTIONARY_STYLE = questionary.Style(
    [
        ("qmark", f"fg:{COLOR_SHOCKING_PINK} bold"),
        ("question", "bold"),
        ("answer", f"fg:{COLOR_WHITE} bold"),
        ("pointer", f"fg:{COLOR_SHOCKING_PINK} bold"),
        ("highlighted", f"fg:{COLOR_SHOCKING_PINK} bold"),
        ("selected", f"fg:{COLOR_WHITE}"),
        ("separator", f"fg:{COLOR_GRAY}"),
        ("instruction", f"fg:{COLOR_GRAY}"),
        ("text", ""),
        ("disabled", f"fg:{COLOR_GRAY} italic"),
    ]
)
